...
**Phase 4 진행 중.** 가방 그리드와 배치까지 완료. 다음은 시너지 4종.
...
**그래서 1-C 가 이 기능의 값어치를 판가름한다.** 지금은 어디에 놓든 효과가
같아서 배치를 고민할 이유가 없다. 시너지가 그것을 만들지 못하면
고유 행동 아이템을 다시 봐야 한다.
...
- **면접 문답 48개 중 답변이 17개다.** (하나는 Phase 4 로 미룬 메모) 이 프로젝트의 목표가 '면접에서 방어 가능한
