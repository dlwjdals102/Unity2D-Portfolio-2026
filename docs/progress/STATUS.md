...
**Phase 4 진행 중.** 시너지 4종까지 완료. 다음은 아이템 15종 채우기.
...
**그래서 1-E 를 새로 넣었다.** 시너지 코드는 더 고칠 것이 없다.
15종을 채운 뒤 같은 질문을 다시 던지되, 이번에는 **'좋은 배치들이 서로 다른
모양인가'** 로 본다. 아이템이 늘면 전수 조사가 안 되므로 판단 방법도 그때 정한다.
...
- **면접 문답 61개 중 답변이 20개다.** (하나는 Phase 4 로 미룬 메모) 이 프로젝트의 목표가 '면접에서 방어 가능한
  게임' 이므로 **가장 밀린 항목이다.**
  시너지 4종은 Q2 와 Q6 만 채우고 Q1, Q3, Q4, Q5 를 남긴 채 닫았다. 코드는 100커밋인데 설명은 그 속도를 못 따라간다.
