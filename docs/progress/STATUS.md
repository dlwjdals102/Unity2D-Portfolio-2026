...
**Phase 2 진행 중.** 이동·대시, 공격, 적 1종 완료. 다음은 피격·사망.

## 지금 하는 일

[추적 근접 적](../specs/enemy-melee.md) **완료.** 다음 기능 착수 대기.

Phase 2 남은 순서: 피격·사망 → 씬 흐름.
**각 기능은 착수 전 `docs/specs/`에 명세를 만든다.**

### 정리해야 할 것

- `EnemyController` 의 Attack 상태에 `Debug.Log` 가 남아 있다.
  피격 기능을 만들 때 실제 공격으로 바꾸고 지운다.
