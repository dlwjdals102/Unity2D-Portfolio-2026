...
**Phase 3 진행 중.** 오브젝트 풀링 완료. 다음은 ScriptableObject 데이터 분리.
...
[ ] 2. ScriptableObject 데이터 분리
[ ] 3. 스탯 · 모디파이어 시스템 (+ EditMode 테스트)
[ ] 4. 적 종류 확장하면서 FSM 구조 재검토
```

2번과 3번은 붙여서 간다. 스탯을 만들려면 값을 어디서 읽을지 정해져 있어야 한다.
순서를 뒤집으면 스탯 시스템을 만들었다가 데이터를 옮기며 다시 손대게 된다.
