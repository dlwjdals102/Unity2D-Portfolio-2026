...
**Phase 4 진행 중.** 인접 시너지까지 완료. 다음은 나머지 시너지 3종.
...
**인접 하나로는 부족했다.** 뭉치면 무조건 이득이라 최적해가 자명하다.
방향이 하나뿐이면 값을 어떻게 바꿔도 마찬가지다.

**새 조건을 발명하지 않고 기획서의 넷을 그대로 만든다.**
모서리는 귀퉁이로 흩어야 이득이라 인접과 정면으로 부딪히고,
같은 행과 같은 열도 서로 배타적이다. 넷이 있으면 선택이 저절로 생긴다.
근거는 [시너지 방향](../decisions/synergy-direction.md)에 있다.
...
- **면접 문답 53개 중 답변이 18개다.** (하나는 Phase 4 로 미룬 메모) 이 프로젝트의 목표가 '면접에서 방어 가능한
  게임' 이므로 **가장 밀린 항목이다.** 코드는 100커밋인데 설명은 그 속도를 못 따라간다.
