...
**Phase 2 진행 중.** 플레이어 이동·대시 완료. 다음은 공격.

## 지금 하는 일

[플레이어 이동·대시](../specs/player-movement.md) **완료.** 다음 기능 착수 대기.

- [x] 명세 작성
- [x] 입력 액션 에셋 + 생성 C# 래퍼 (`PlayerControls`)
- [x] 게임플레이 씬
- [x] `PlayerInputReader` — 입력을 읽어 의도만 노출
- [x] `PlayerMotor` — 의도를 받아 실제로 움직임
- [x] 플레이모드 체크리스트 12개 통과

**이동·대시 완료.** 다음은 공격 → 적 1종 → 피격·사망 → 씬 흐름.
